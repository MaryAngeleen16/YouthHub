...
const EventDetails = () => {
  ...
  try {
    if (!loggedInUserId) {
      toast.error('You need to be logged in to join the event.');
      return;
    }
      ...
      <div className="main-content-youth">
        {event ? (
          <div>
            ...
            <button onClick={() => handleAudienceCapacityUpdate('unjoin')} className='unjoinButton' enabled={loading || isUserJoined}>Unjoin</button>
            </div>
